...
soit dériver de Exception pour bénéficier de __str__()
soit standalone mais définir un constructeur avec self.message = message et éventuellement implémenter __str__() pour retourner ce message
...
super().__init__() alors que non défini dans la classe parent            # acceptable mais rien ne se produit
...
*erreurs courantes*
ValueError                                  conversion, index() non trouvé                     int("three")    [1,2,3].index(4)
